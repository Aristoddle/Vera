...
- CoreML reranker probe fails on Apple Silicon. Current builds use the reranker FP16 export because the quantized Jina ONNX model contains ops the CoreML execution provider cannot execute. If you set up CoreML on an older Vera version, run `vera repair --onnx-jina-coreml` to fetch `onnx/model_fp16.onnx`.
